...
PIXI
    ...
    .add(
        'bulletsTile', 'res/assets/img/bullets.png'
    )
    .load(setup);
...
Keys.DOWN.release = () => {
    socket.emit('release-button', 'down');
};
...
function updateObstacles(new_obstacles) {
    ...
    console.log(should_update, new_obstacles, app.state.obstacles);

    if (!should_update) {
        return;
    }

    if (app.state.obstacles != null && app.state.obstacles.length > 0) {
        for (let i = 0; i < app.state.obstacles.length; i++) {
            app.stage.removeChild(app.state.obstacles[i].sprite);
        }
    }
    ...
    for (let i = 0; i < new_obstacles.length; i++) {
        ...
        obstacle.sprite = genObstacle(obstacle.x, obstacle.y, obstacle.width, obstacle.height);
        defaultAddSpritesToStage([obstacle.sprite]);
    }

    app.state.obstacles = new_obstacles;
}

function updateBullets(new_bullets) {
    let should_update = new_bullets != null;

    if (!should_update) {
        return;
    }

    let new_bullets_ids_set = new Set(new_bullets.map(b => b.id));
    let old_bullets_ids_set = new Set(new Set((app.state.bullets || []).map(b => b.id)));

    let bullets_ids_to_remove_arr = new Array(...old_bullets_ids_set).filter(id => !new_bullets_ids_set.has(id));

    console.log(new_bullets_ids_set, old_bullets_ids_set, bullets_ids_to_remove_arr);
    let saved_old_bullets_sprites_obj = {};

    if (app.state.bullets != null && app.state.bullets.length > 0) {
        for (let i = 0; i < app.state.bullets.length; i++) {
            let should_remove_bullet = bullets_ids_to_remove_arr.indexOf(app.state.bullets[i].id) !== -1;

            if (should_remove_bullet) {
                console.log('REMOVED bullet', app.state.bullets[i].id);
                app.stage.removeChild(app.state.bullets[i].sprite);
            } else {
                saved_old_bullets_sprites_obj[app.state.bullets[i].id] = app.state.bullets[i].sprite;
            }
        }
    }

    for (let i = 0; i < new_bullets.length; i++) {
        let bullet = new_bullets[i];

        if (saved_old_bullets_sprites_obj[bullet.id] == null) {
            bullet.sprite = genBullet(bullet.x, bullet.y);
            defaultAddSpritesToStage([bullet.sprite]);
        } else {
            bullet.sprite = saved_old_bullets_sprites_obj[bullet.id];
        }
    }

    app.state.bullets = new_bullets;
}
...
function genObstacle(pos_x, pos_y, size_x, size_y) {
    ...
    let obstacleSprite = new PIXI.extras.TilingSprite(obstacleTexture, size_x, size_y);

    obstacleSprite.x = pos_x;
    obstacleSprite.y = pos_y;
...
function genBullet(pos_x, pos_y) {
    let bulletRect = new PIXI.Rectangle(0, 5 * 32, 32, 32);
    let bulletTexture = new PIXI.Texture(PIXI.utils.TextureCache.bulletsTile, bulletRect);

    bulletTexture.frame = bulletRect;

    let bulletSprite = new PIXI.Sprite(bulletTexture);
    bulletSprite.animationFrameId = 0;

    bulletSprite.animate = function () {
        let h = 32;
        let w = 32;
        let y = 5;

        this.animationFrameId = ((this.animationFrameId + 1) % 10);
        let x = this.animationFrameId * w;

        console.log('bulletX', x);

        bulletSprite.frame = new PIXI.Rectangle(x, y, w, h);
    };

    bulletSprite.x = pos_x;
    bulletSprite.y = pos_y;
