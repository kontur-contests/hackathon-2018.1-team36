...
class Game {
    ...
    update(dt) {
        ...
        this._bullets.forEach(b => b.move(dt / config.TIME));
        ...
        this._tanks.forEach(function (tank, tank_id) {
            ...
            // then - move
            tank.move(dt / config.TIME);
            tank.x = Math.min(tank.x, self._width);
            tank.y = Math.min(tank.y, self._height);
    ...
    createTank(x, y) {
        // create in middle of world
        let _x = x || this._width / 2;
        let _y = y || this._height / 2;
        let tank = Generator.new_tank(this._width, this._height, _x, _y);
    ...
    currentState() {
        return {
            ...
            'bullets': this._bullets.map(b => ({ x: b.x, y: b.y, id: b.id })),
